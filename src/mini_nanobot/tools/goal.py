...
用 LangGraph 的 `Command` 直接更新 `state["goal_state"]`——LangGraph 里「工具修改
自定义状态」的标准写法。

安全边界：只有服务层识别到显式 `/goal` 请求时，才把本轮 `goal_creation_allowed`
设为真。工具会再次检查该状态，不能只靠模型遵守提示词。
"""
...
def update_goal(
    ...
    if action == "replace":
        # 安全规则：没有进行中的目标时不许用 replace 变相新建目标（绕过 /goal 授权）。
        if goal_state.get("status") != "active":
            return Command(
                update={
                    "messages": [
                        ToolMessage(
                            content="拒绝替换目标：当前没有进行中的目标。",
                            tool_call_id=tool_call_id,
                        )
                    ]
                }
