...
内层 agent 负责标准 ReAct 循环（重试、压缩、调用预算、动态提示词由 middleware 组合）；
外层 StateGraph 只保留 mini-nanobot 特有的 sustained-goal 续跑。这样不重复实现
LangChain 已经提供的模型/工具循环，同时仍能清楚看到业务状态机。
"""
...
def _build_graph(
    ...
    async def prepare_run(state: AgentState, runtime: Runtime[AgentContext]) -> dict:
...
async def create_app(cfg: AppConfig) -> AsyncIterator[AppRuntime]:
    ...
    是 async context manager，因为 `AsyncSqliteSaver` 需要在 `async with` 块里维持
    数据库连接——退出作用域，连接会被正确关闭。
    """
    llm = _build_llm(cfg)
