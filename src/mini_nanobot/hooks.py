...
让「在关键节点插入自定义逻辑」不需要改 graph.py——写一个新的 Hook 子类，塞进
hooks 列表就行。模型/工具/压缩等执行期钩子交给 LangChain middleware；这里只保留
产品层的一轮运行事件，避免维护两套重复生命周期。
"""
...
class LoggingHook(AgentHook):
    """最基础的实现：把每个生命周期节点打印到日志，方便观察循环过程。"""
...
class HookRegistry:
    """把多个 hook 合并成一组，逐个调用。任何 hook 抛异常都不应打断主流程。"""
