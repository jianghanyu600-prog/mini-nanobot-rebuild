...
对应 nanobot 的 `AgentLoop`。完整版有 8 状态状态机（RESTORE → COMPACT → COMMAND →
BUILD → RUN → SAVE → RESPOND → DONE），这里简化成一层显式判断：
「这是斜杠命令还是普通对话」，对应 COMMAND 状态；其余交给 LangGraph。
"""
...
class AgentService:
    ...
    async def _run_streamed(
        ...
        """用 `stream_mode="messages"` 拿到 token 级增量，边生成边发出去。"""
        parts: list[str] = []
